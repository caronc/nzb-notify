...
WEBBASE_LOOKUP_TABLE = (
    ...
    (
        'Google Mail',
        re.compile('^(?P<id>[^@]+)@(?P<domain>gmail\.com)$', re.IGNORECASE),
        {
            'port': 587,
    ...
    (
        'Microsoft Hotmail',
        re.compile('^(?P<id>[^@]+)@(?P<domain>(hotmail|live)\.com)$', re.IGNORECASE),
        {
            'port': 587,
    ...
    (
        'Yahoo Mail',
        re.compile('^(?P<id>[^@]+)@(?P<domain>yahoo\.(ca|com))$', re.IGNORECASE),
        {
            'port': 465,
    ...
    (
        'Custom',
        re.compile('^(?P<id>[^@]+)@(?P<domain>.+)$', re.IGNORECASE),
        {
...
class NotifyEmail(NotifyBase):
    ...
    def __init__(self, to,  notify_format, **kwargs):
        ...
        self.from_name = kwargs.get('name', 'NZBGet Notification')
        ...
        if not IS_EMAIL_RE.match(self.to_addr):
            raise TypeError('Invalid ~To~ email format: %s' % \
                            self.to_addr)
        ...
        if not match:
            ...
            raise TypeError('Invalid ~From~ email format: %s' % \
                            self.to_addr)
    ...
    def _notify(self, title, body, **kwargs):
        ...
        self.logger.debug('Email From: %s <%s>' % (self.from_addr, self.from_name))
